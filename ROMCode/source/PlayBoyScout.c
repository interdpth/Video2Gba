    ...
    *interrupt_enable = 0;
    ...
    if ((*REG_IF & INTERRUPT_VBLANK) == INTERRUPT_VBLANK) {
        ...
        if (channel_a_vblanks_remaining == 0) {
            ...
            *dma1_control = 0;
            *dma1_source = (unsigned int) lztownwav;
...
if(FrameCounter==0)
{
play_sound(lztownwav, lztownwav_size, 10512, 'A');
}
